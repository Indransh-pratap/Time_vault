import { useEffect, useState } from 'react';
import api from '../lib/api';
import { Target as TargetIcon, Plus, ChevronUp } from 'lucide-react';
import { toast } from 'react-hot-toast';

interface ObjectiveData {
  _id: string;
  title: string;
  targetValue: number;
  currentValue: number;
  progressPercent: number;
}

const TargetsDashboard = () => {
  const [objectives, setObjectives] = useState<ObjectiveData[]>([]);
  const [loading, setLoading] = useState(true);

  // New objective state
  const [title, setTitle] = useState('');
  const [targetValue, setTargetValue] = useState<number | ''>('');

  const fetchObjectives = () => {
    setLoading(true);
    api.get('/objectives')
       .then(res => setObjectives(res.data))
       .catch((err) => {
         console.error(err);
         toast.error("Failed to load objectives");
       })
       .finally(() => setLoading(false));
  };

  useEffect(() => {
    fetchObjectives();
  }, []);

  const handleAddObjective = async () => {
    if (!title || !targetValue) {
      toast.error('Fill in all objective fields');
      return;
    }
    try {
      await api.post('/objectives', { title, targetValue: Number(targetValue) });
      setTitle('');
      setTargetValue('');
      fetchObjectives();
      toast.success('Objective created', {
        style: { border: '1px solid #E50914', background: '#0B0C10', color: '#fff' }
      });
    } catch (e) {
      toast.error('Failed to create objective');
    }
  };

  const addProgress = async (id: string) => {
    try {
      await api.put(`/objectives/${id}/progress`, { amount: 1 });
      fetchObjectives();
    } catch (e) {
      toast.error('Failed to update progress');
    }
  };

  return (
    <div className="flex flex-col gap-6 w-full p-2">
      <h3 className="text-xl font-bold uppercase tracking-widest text-white border-b-2 border-gray-900 pb-3 flex justify-between items-center">
        <span>Strategic Objectives</span>
        <TargetIcon size={20} className="text-[#E50914] drop-shadow-[0_0_5px_#E50914]" />
      </h3>

      {/* Input Section */}
      <div className="bg-[#111] border border-gray-800 p-4 rounded-sm flex flex-col md:flex-row gap-3 items-end md:items-center shadow-[0_4px_15px_rgba(0,0,0,0.5)]">
        <div className="flex-1 w-full">
          <label className="text-xs text-gray-500 font-mono tracking-widest uppercase mb-1 block">Objective Title</label>
          <input 
            type="text" 
            placeholder="e.g. 10 hours coding" 
            value={title} 
            onChange={e => setTitle(e.target.value)}
            className="w-full bg-[#050505] border border-gray-700 text-white px-3 py-2 rounded focus:outline-none focus:border-[#E50914] transition-colors font-mono text-sm"
          />
        </div>
        <div className="w-full md:w-32">
          <label className="text-xs text-gray-500 font-mono tracking-widest uppercase mb-1 block">Target</label>
          <input 
            type="number" 
            value={targetValue} 
            placeholder="e.g. 10"
            onChange={e => setTargetValue(Number(e.target.value))}
            className="w-full bg-[#050505] border border-gray-700 text-white px-3 py-2 rounded focus:outline-none focus:border-[#E50914] transition-colors font-mono text-sm"
          />
        </div>
        <button 
          onClick={handleAddObjective}
          className="w-full md:w-auto mt-4 md:mt-0 flex items-center justify-center gap-2 px-6 py-2 bg-[#E50914]/10 border border-[#E50914] text-[#E50914] hover:bg-[#E50914] hover:text-white transition-all font-mono uppercase tracking-widest text-sm rounded shadow-[0_0_10px_rgba(229,9,20,0.2)]"
        >
          <Plus size={16} /> Add 
        </button>
      </div>

      <div className="flex flex-col gap-5">
        {loading ? (
          [...Array(2)].map((_, i) => (
            <div key={i} className="h-20 bg-[#111] animate-pulse border border-gray-800 rounded-sm"></div>
          ))
        ) : objectives.length === 0 ? (
          <div className="text-[#E50914]/70 bg-[#E50914]/5 font-mono text-sm tracking-widest uppercase text-center py-8 border-2 border-dashed border-[#E50914]/30 rounded-sm">
            No active overriding objectives.
          </div>
        ) : (
          objectives.map((t) => (
            <div key={t._id} className="bg-[#111] border border-gray-800 p-4 rounded-sm relative overflow-hidden group hover:border-gray-500 transition-colors">
              <div className="flex justify-between items-center mb-3 relative z-10">
                <div className="flex flex-col">
                  <span className="text-sm font-bold text-gray-200 uppercase tracking-widest font-mono">{t.title}</span>
                  <span className="text-xs text-gray-500 font-mono tracking-widest mt-1">
                    Completed: {t.currentValue} / {t.targetValue} &nbsp;|&nbsp; Remaining: {t.targetValue - t.currentValue > 0 ? t.targetValue - t.currentValue : 0}
                  </span>
                </div>
                <div className="flex items-center gap-4">
                  <button 
                    onClick={() => addProgress(t._id)}
                    className="p-1.5 border border-gray-600 rounded text-gray-400 hover:text-green-500 hover:border-green-500 transition-colors"
                    title="Add +1 Progress"
                  >
                    <ChevronUp size={16} />
                  </button>
                  <span className="text-sm text-[#E50914] font-mono tracking-widest font-bold">{t.progressPercent}%</span>
                </div>
              </div>
              <div className="w-full bg-black h-2 rounded-full overflow-hidden relative z-10 border border-gray-800">
                <div 
                  className="bg-gradient-to-r from-red-900 to-[#E50914] h-full transition-all duration-1000 ease-out shadow-[0_0_10px_#E50914]" 
                  style={{ width: `${t.progressPercent}%` }}
                />
              </div>
            </div>
          ))
        )}
      </div>
    </div>
  );
};

export default TargetsDashboard;
